...
public class ConwaysRuleSet implements RuleSetInterface {
	private boolean wrapped;
	
	public ConwaysRuleSet(){
		this.wrapped = false;
	}
	
	public ConwaysRuleSet(boolean wrapped){
			...
			for (int i = 0; i < grid.getHeight(); i++) {
				if (grid.getGridCell(i, j) == true) {
					if (this.numNeighbors(i, j, grid) < 2) {
						temp.setGridCell(i, j, false);
					} else if (this.numNeighbors(i, j, grid) > 3) {
						temp.setGridCell(i, j, false);
					} else {
						temp.setGridCell(i, j, true);
					}
				} else {
					if (this.numNeighbors(i, j, grid) == 3) {
						temp.setGridCell(i, j, true);
					}
				}
			}
